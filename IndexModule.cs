            ...
            try
            {
                ...
                Get["/blacklist"] = parameters =>
                {
                    ...
                    if (mt4_ids.Count > 0)
                    {
                        ...
                        client.Connect(ConfigurationManager.AppSettings["Connect"]);//tcp://127.0.0.1:1990
                    ...
                    try
                    {
                        ...
                        if (data != null)
                        {
                            ...
                            using (var cmd = DBUtility.ExecuteReader(null, CommandType.Text, sesql, new MySqlParameter("@mt4_real", data)))
                            {
                                ...
                                {
                                    HasData = true;
                                    d.username = cmd["username"];
                                    d.usercode = cmd["user_code"];
                                    d.mt4_real = cmd["mt4_real"];
                                    d.date = DateTime.Now;
                                }
                            }
                            if (HasData)
                ...
                Get["unwraptelnum"] = pramamters =>
                {
                    ...
                    if (phone!=null)
                    {
                        var sql = "SELECT count(*)as count FROM tiger.user where phone=@phone";
                        var obj = DBUtility.ExecuteScalar(null, CommandType.Text, sql, new MySqlParameter("@phone", phone));
                        if (int.Parse(obj.ToString())!=0)
                        {
                            var delesql = "update tiger.user set phone=@value where phone=@phone";
                            count = DBUtility.ExecuteNonQuery(null, CommandType.Text, delesql, new MySqlParameter[] { new MySqlParameter("value","-1"), new MySqlParameter("@phone",phone) });
                            if (count > 0)
                            {
                                result = "True";
                            }
                ...
                Get["unwraptel"] = pramamters =>
                {
                    return View["unwraptel"];
                };
                #endregion
            }
            catch (Exception e)
